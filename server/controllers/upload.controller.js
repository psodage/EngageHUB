...
export async function ingestRemoteSocialMedia(req, res) {
  ...
  try {
    const url = await ingestRemoteUrlToUploads(remoteUrl);
    ...
    const status = error?.message?.includes("not allowed") ? 400 : 502;
    return errorResponse(
      res,
      error?.message || "Could not download media from that URL.",
      status,
      status === 400 ? "validation_error" : "remote_ingest_failed"
    );
  }
}
